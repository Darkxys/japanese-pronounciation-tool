...
from utils import (
    parse_wav_file_to_sound_obj,
    ts_sequences_to_index,
    segment_speech,
    znormed,
...
class SpeechRecord:
    ...
    def __init__(
        ...
        self.pitch = (
            self.snd.to_pitch(time_step=PITCH_TIME_STEP).kill_octave_jumps().smooth()
        )
        ...
        self.intensity = self.snd.to_intensity(
            MINIMUM_PITCH, time_step=INTENSITY_TIME_STEP
        )
        ...
        self.begin_idx, self.end_idx = ts_sequences_to_index(
            [self.begin_ts, self.end_ts], self.intensity.xs()
        )
        logging.debug(f"Voice detected from {self.begin_ts}s to {self.end_ts}s")

        self.phonemes = None
        if self.sentence:
            self.phonemes = segment_speech(
                self.wav_filename,
                self.sentence.julius_transcript,
                self.begin_ts,
                self.end_ts,
            )
    ...
    def align_with(
        self,
        ref_rec: "SpeechRecord",
        method: AlignmentMethod = AlignmentMethod.phonemes,
    ):
        ...
        if method == AlignmentMethod.phonemes:
            ...
            x = phonemes_to_step_function(
                student_rec.phonemes, student_rec.timestamps_for_alignment
            )
            y = phonemes_to_step_function(
                teacher_rec.phonemes, teacher_rec.timestamps_for_alignment
            )
    ...
    def znormed_intensity(self):
        return znormed(self.intensity.values[0, self.begin_idx : self.end_idx])

    @property
    def aligned_pitch(self):
        if self.align_ts is None:
            return self.pitch_freq
