...
def segment_speech(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        save_cropped_audio(
            wav_filename, os.path.join(tmpdir, "tmp.wav"), begin_ts, end_ts
        )
...
def save_cropped_audio(
    src_wav_filename: str, dst_wav_filename: str, begin_ts: float, end_ts: float
):
    p = subprocess.Popen(
        [
            "ffmpeg",
...
def parse_wav_file_to_sound_obj(wav_filename: str) -> parselmouth.Sound:
    ...
    if snd.sampling_frequency != 16000:
        raise ValueError(
            f"Sampling frequency should be 16kHz, " f"not {snd.sampling_frequency}Hz !"
        )
    if snd.n_channels != 1:
        raise ValueError(
            f"Should only have 1 audio channel, " f"not {snd.n_channels} !"
        )
    return snd
...
def phonemes_to_step_function(
    phonemes: List[Tuple[float, float, str]], timestamps: Iterable[float]
) -> List[float]:
    """
